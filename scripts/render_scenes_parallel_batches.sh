#!/usr/bin/env bash
# Enhanced parallel rendering: splits scene dirs for each input_id into batches and runs as many jobs as cores allow
set -euo pipefail
...
for id in "${ids[@]}"; do
  echo "Processing input_id=$id..."
  mapfile -t scene_dirs < <(find "$scene_base_dir" -maxdepth 1 -type d -name "${id}*" | sort)
  num_scenes=${#scene_dirs[@]}
  if [ "$num_scenes" -eq 0 ]; then
    echo "No scenes found for input_id=$id"
    continue
  fi
  ...
  while [ $batch_start -lt $num_scenes ]; do
    ...
    # Pass scene directories as a comma-separated list
    scene_dirs_arg=$(IFS=,; echo "${batch[*]}")
    env "${env_vars[@]}" taskset -c "$core_range" python -m mgs.cli.render_scene_point_label input_id="$id" scene_dirs="$scene_dirs_arg" &
    batch_pids+=("$!")
